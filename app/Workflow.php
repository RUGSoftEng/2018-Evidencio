...
class Workflow extends Model
{
    ...
    public function search($title){
        return Workflow::join('users', 'users.id', '=', 'author_id')->where('title', 'LIKE', '%'.$title.'%')->orWhere('description', 'LIKE', '%'.$title.'%')->select('users.id','users.first_name AS fname', 'users.last_name as lname', 'workflows.*')->get();
